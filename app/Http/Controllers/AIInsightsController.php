...
class AIInsightsController extends Controller
{
    ...
    public function autoGenerateAll(): JsonResponse
    {
        try {
            // Get events with feedback but no insights
            $events = Event::whereHas('feedbackResponses')
                ->whereDoesntHave('aiInsights')
                ->get();
        
            if ($events->isEmpty()) {
                return response()->json([
                    'success' => true,
                    'message' => 'All events with feedback already have insights',
                    'processed' => 0
                ]);
            }
            ...
            foreach ($events as $event) {
                try {
                    $result = $this->aiService->generateInsights($event, false);
                    $results[] = [
                        'event_id' => $event->id,
    ...
    /**
     * Get events that need insights (have feedback but no insights)
     */
    public function getEventsNeedingInsights(): JsonResponse
    {
        try {
            $events = Event::whereHas('feedbackResponses')
                ->whereDoesntHave('aiInsights')
                ...
                ->map(function ($event) {
                    return [
                        ...
                        'feedback_count' => $event->feedbackResponses->count()
                    ];
                });
